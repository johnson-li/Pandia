...
echo 'Server started'
tmux send-key -t $session:2 "sudo ip netns exec ${ns} ~/Workspace/Pandia/bin/peerconnection_client_headless --port ${port} --dump_path ${dump} --name receiver --receiving_only true --force_fieldtrials=WebRTC-FlexFEC-03-Advertised/Enabled/WebRTC-FlexFEC-03/Enabled/ 2> $log" Enter
